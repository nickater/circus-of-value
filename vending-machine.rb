class VendingMachine
    ...
    def purchaseItem(selector)
        ...
        @items.each_with_index do |item, index|
            if item.selector == selector.upcase
                item_name = item.name;
                if item.quantity > 0
                    item_found = true;
    ...
    def cashOut
        ...
        nickels = 0
        pennies = 0
        while "#{'%.2f' % @funds}".to_f >= 0.25
            @funds -= 0.25
            quarters += 1
        end
        while "#{'%.2f' % @funds}".to_f >= 0.10
            @funds -= 0.10
            dimes += 1
        end
        while "#{'%.2f' % @funds}".to_f >= 0.05
            @funds -= 0.05
            nickels += 1
        end
        while "#{'%.2f' % @funds}".to_f > 0
            @funds -= 0.01
            pennies += 1
        end
